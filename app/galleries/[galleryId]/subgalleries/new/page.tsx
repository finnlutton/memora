...
export default function NewSubgalleryPage() {
  ...
  const { getGallery, createSubgallery, hydrated } = useMemoraStore();
    ...
    <AppShell>
      <section className="mb-6">
        ...
        <h1 className="mt-3 font-serif text-5xl text-[color:var(--ink)]">
          Add a chapter to {gallery.title}
        </h1>
      </section>
      <SubgalleryForm
        galleryId={gallery.id}
        onSubmit={(value) => {
          const subgalleryId = createSubgallery(gallery.id, value);
          router.push(`/galleries/${gallery.id}/subgalleries/${subgalleryId}`);
        }}
      />
    </AppShell>
  );
